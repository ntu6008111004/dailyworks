...
const ReviewDialog = ({ briefing, users, onClose, onChanged }) => {
  ...
  const action = async (type) => {
    if (['needs_revision', 'rejected', 'severe_error', 'extra_work', 'extend_deadline'].includes(type) && !comment.trim()) { toast.error('กรุณาระบุรายละเอียดหรือเหตุผลก่อน'); return; }
    if (['rejected', 'severe_error'].includes(type) && targetUserIds.length === 0) { toast.error('กรุณาเลือกผู้รับงานที่ต้องหักคะแนน'); return; }
    setSaving(true);
    try {
...
const ActionButton = ({ color, icon, label, detail, disabled, onClick }) => {
  ...
  return <button disabled={disabled} onClick={onClick} className={`flex items-center justify-between gap-2 rounded-xl px-3 py-2.5 text-left text-white disabled:opacity-50 ${styles[color]}`}><span className="inline-flex items-center gap-1.5 text-xs font-black">{icon}{label}</span><span className="text-[9px] font-semibold opacity-80">{detail}</span></button>;
};
