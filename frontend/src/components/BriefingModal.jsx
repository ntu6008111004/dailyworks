...
import {
  ...
  Link as LinkIcon, Loader2, PlayCircle, Plus, Save, Send, UploadCloud, UserRound, X
} from 'lucide-react';
...
export const BriefingModal = ({ briefing, onClose, onSaved, allUsers = [] }) => {
  ...
  // Anyone who may edit the brief (creator, department head, admin — never a
  // recipient) may repick the score in any status until the head approves
  // the work as เสร็จสิ้น.
  const scoreLocked = isBriefingScoreLocked(fullBriefing?.Status || briefing?.Status);
  const canEditPoints = canEditBrief && !scoreLocked;
  ...
  // One tap tells the person who briefed the work that it has started: the
  // briefing flips to กำลังทำ and Realtime carries it to their screen.
  const handleStartWork = async () => {
    if (!briefing?.ID || saving || uploading) return;
    setSaving(true);
    try {
      await apiService.startBriefingWork(briefing.ID);
      toast.success('เริ่มงานแล้ว สถานะเปลี่ยนเป็น “กำลังทำ” และแจ้งผู้มอบหมายให้ทราบ', { position: 'bottom-right' });
      await loadData(); onSaved();
    } catch (error) { toast.error(error.message || 'เริ่มงานไม่สำเร็จ', { position: 'bottom-right' }); }
    finally { setSaving(false); }
  };
 ...
 */
const ReviewNoteList = ({ notes, onPreview }) => {
  ...
  return <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
    ...
    <ol className="space-y-2">{notes.map((note) => <li key={note.id} className={`rounded-xl border p-3 ${NOTE_TONES[note.tone] || NOTE_TONES.slate}`}>
      ...
      {note.images?.length > 0 && <div className="mt-2 grid grid-cols-3 gap-2 sm:grid-cols-6">{note.images.map((image, index) => <img key={`${image}-${index}`} src={image} alt={`จุดที่ต้องแก้ ${index + 1}`} onClick={() => onPreview?.(image)} className="aspect-square w-full cursor-zoom-in rounded-lg border border-white/60 bg-white object-cover" />)}</div>}
